...
model = LogisticModel(init_learning_rate, decay_steps, decay_rate)
...
def validataion():
    ...
    for i in range(0, len(loader.test_Y), step_size):
        ...
        output, y_logit = sess.run([model.prediction, model.logits_pred],
                                   feed_dict={model.e: batch_E, model.y: batch_Y,
    ...
    for epoch in range(epoch_num):
        ...
        for iter, idx in enumerate(range(0, loader.training_size, batch_size)):
            ...
            batch_loss, y_pred, y_logits, accuracy, _ = sess.run(
                [model.loss, model.prediction, model.logits_pred, model.accuracy, model.optimizer],
                feed_dict={model.e: batch_E, model.y: batch_Y,
